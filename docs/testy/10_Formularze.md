...
-   Wyłączone elementy wejściowe nie otrzymują fokus klawiatury, nie można ich wybrać ani modyfikować. Nie są one uwzględniane w tym teście.
...
3.  Sprawdź, czy podano dodatkowe wskazówki, jak poprawić błędy w polach formularza, które nie zagroziłyby bezpieczeństwu ani przeznaczeniu treści (np. pryzkłady poprawnych danych, wskazówki, jak poprawić dane wprowadzone przez użytkownika).
